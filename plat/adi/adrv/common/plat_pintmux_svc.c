...
static int plat_pintmux_smc_map(unsigned int gpio, bool is_secure, bool pos_mask, uintptr_t base_addr)
{
	...
	if (lane < 0) {
		plat_runtime_error_message("PINTMUX service: Failed to map GPIO_%s_%u - already mapped or no lanes available.", is_secure ? "S" : "NS", gpio);
		return lane;
	}
	irq = plat_pintmux_lane_to_irq(lane, base_addr);
	if (irq == 0) {
		plat_runtime_error_message("PINTMUX service: Failed to translate GPIO_%s_%u to IRQ.", is_secure ? "S" : "NS", gpio);
		return irq;
	}
...
uintptr_t plat_pintmux_smc_handler(unsigned int smc_fid, u_register_t x1, u_register_t x2, u_register_t x3, u_register_t x4, void *cookie, void *handle, u_register_t flags)
{
	...
	plat_runtime_warn_message("PINTMUX service: Unexpected FID %d", fid);
