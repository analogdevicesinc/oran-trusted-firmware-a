...
#define SDMMC_CLKCR_HWFC_EN             BIT(8)
#define SDMMC_CLKCR_SELCLKRX_0          BIT(20)

/* SDMMC command register */
#define SDMMC_CMDR_CMDTRANS             BIT(12)
#define SDMMC_CMDR_CMDSTOP              BIT(12)
#define SDMMC_CMDR_WAITRESP             GENMASK(7, 6)
#define SDMMC_CMDR_WAITRESP_SHORT       BIT(6)
#define SDMMC_CMDR_WAITRESP_SHORT_NOCRC BIT(6)
#define SDMMC_CMDR_CPSMEN               BIT(10)

/* SDMMC data control register */
#define SDMMC_DCTRLR_DTEN               BIT(0)
#define SDMMC_DCTRLR_DTDIR              BIT(1)
#define SDMMC_DCTRLR_DTMODE             BIT(2)
...
static struct systemc_pl180_params sdmmc2_params;
...
static int stm32_sdmmc2_send_cmd_req(struct mmc_cmd *cmd)
{
	...
	case MMC_ACMD(41):
		...
		break;
	/* BKN: Add CMD 24 and 25 to the list of xfer commands */
	case MMC_CMD(24):
	case MMC_CMD(25):
	case MMC_ACMD(51):
		cmd_reg |= SDMMC_CMDR_CMDTRANS;
	...
	/* BKN: For write commands, disable the 'data read' setting configured
	 * in stm32_sdmmc2_prepare() and setup a 'data write' configuration.
	 * This is probably not the best way to do this, but is sufficient for
	 * the SystemC model driver.
	 */
	if (cmd->cmd_idx == MMC_CMD(24) || cmd->cmd_idx == MMC_CMD(25)) {
		udelay(10u);
		mmio_write_32(base + SDMMC_DCTRLR, mmio_read_32(base + SDMMC_DCTRLR) & ~0x1);
		udelay(10u);
		mmio_write_32(base + SDMMC_DCTRLR, (mmio_read_32(base + SDMMC_DCTRLR) & ~0x2) | 0x1);
	}

	if ((cmd->resp_type & MMC_RSP_BUSY) != 0U)
		mmio_write_32(base + SDMMC_DTIMER, UINT32_MAX);

	/* BKN: Save the last command for use in read() */
	lastCmd = cmd->cmd_idx;
...
static int stm32_sdmmc2_prepare(int lba, uintptr_t buf, size_t size)
{
	...
	data_ctrl |= builtin_ctz(arg_size) << SDMMC_DCTRLR_DBLOCKSIZE_SHIFT;

	/* BKN: ST driver doesn't use the xfer enable flag, but the PL180 model
	 * requires it.
	 */
	data_ctrl |= SDMMC_DCTRLR_DTEN;
...
static void systemc_pl180_read_fifo(uint32_t *val, uintptr_t addr)
{
	*val = mmio_read_32(addr);
}

static void systemc_pl180_write_fifo(uint32_t *val, uintptr_t addr)
{
	mmio_write_32(addr, *val);
}

static uint8_t fakeExtCsd[512];
static int stm32_sdmmc2_read(int lba, uintptr_t buf, size_t size)
{
	...
	/* BKN: Can't seem to get SEND_EXT_CSD working with the FVP model
	 * for the PL180/MMC, so fake out a response from the chip and return
	 * that data. This should be sufficient for what we need, but of course
	 * could never work in a production driver.
	 */
	if (lastCmd == MMC_CMD(8)) {
		uint32_t fakeChipSize = 4194304; // 2GB @ 512 byte sector size
		fakeExtCsd[CMD_EXTCSD_SEC_CNT] = (fakeChipSize & 0xFF);
		fakeExtCsd[CMD_EXTCSD_SEC_CNT + 1] = (fakeChipSize >> 8) & 0xFF;
		fakeExtCsd[CMD_EXTCSD_SEC_CNT + 2] = (fakeChipSize >> 16) & 0xFF;
		fakeExtCsd[CMD_EXTCSD_SEC_CNT + 3] = (fakeChipSize >> 24) & 0xFF;
		memcpy(buffer, fakeExtCsd, 512);
		return 0;
	}
	...
	do {
		...
		if (size < (8U * sizeof(uint32_t))) {
			if ((mmio_read_32(base + SDMMC_DCNTR) > 0U) &&
				...
				systemc_pl180_read_fifo(buffer, fifo_reg);
				buffer++;
			}
...
static int stm32_sdmmc2_write(int lba, uintptr_t buf, size_t size)
{
	/* BKN: This is a very simple implementation of write() that would
	 * never work in a production driver. There is no status checking,
	 * no waiting for the FIFO to empty, etc. It is intended only for the
	 * very basic write functionality needed in the SystemC model.
	 */
	uint32_t *fifobuf = (uint32_t *)buf;
	uintptr_t base = sdmmc2_params.reg_base;

	while (size >= sizeof(uint32_t)) {
		systemc_pl180_write_fifo(fifobuf, base + SDMMC_FIFOR);
		fifobuf++;
...
int systemc_pl180_mmc_init(struct systemc_pl180_params *params)
{
	...
	memcpy(&sdmmc2_params, params, sizeof(struct systemc_pl180_params));
	sdmmc2_params.device_info = &mmc_info;
	memcpy(&mmc_info, params->device_info, sizeof(mmc_info));
